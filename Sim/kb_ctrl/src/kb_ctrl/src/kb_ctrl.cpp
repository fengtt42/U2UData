...
int main(int argc, char** argv)
{
    ...
    ros::init(argc, argv, "kb_dev"); // 初始化ros 节点，命名为 kb_dev
...
{  
    ...
    ros::Timer timer = nh->createTimer(ros::Duration(0.05), &KBCtrl::ctrl_cb, this);
    ros::spin();
}
...
{
    int k = 600
    if(key_status[0])delta_yaw = -k;
    else 
    {
        if(!key_status[2])delta_yaw = 0;
    }
    if(key_status[1])delta_x = k;
    else 
    {
        if(!key_status[5])delta_x = 0;
    }
    if(key_status[2])delta_yaw = k;
    else 
    {
        if(!key_status[0])delta_yaw = 0;
    }
    if(key_status[3])delta_z = -k;
    else 
    {
        if(!key_status[7])delta_z = 0;
    }
    if(key_status[4])delta_y = -k;
    else 
    {
        if(!key_status[6])delta_y = 0;
    }
    if(key_status[5])delta_x = -k;
    else 
    {
        if(!key_status[1])delta_x = 0;
    }
    if(key_status[6])delta_y = k;
    else 
    {
        if(!key_status[4])delta_y = 0;
    }
    if(key_status[7])delta_z = k;
    else 
    {
    ...
    float new_yaw = set_pos.request.yaw + delta_yaw /2 *0.2;
    ...
    set_pos.request.x += delta_x;
    set_pos.request.y += delta_y;
